...
function Demos() {
    ...
    const fetchDemos = async () => {
        try {
            setLoading(true);
            setError('');
            // Updated endpoint to /api/calls/demos/
            const res = await apiClient.get('/calls/demos/');
            ...
            setError(`Failed to load demos: ${e.response?.data?.detail || e.message}`);
            setRows([]);
        } finally {
            setLoading(false);
        }
                    ...
                    <TableBody>
                        {Array.isArray(rows) && rows.map((row) => (
                            <TableRow key={row.id}>
                                ...
                                <TableCell>{row.lead_phone || '-'}</TableCell>
                        ...
                        {Array.isArray(rows) && rows.length === 0 && (
                            <TableRow>
                                <TableCell colSpan={5} align="center">No demos scheduled</TableCell>
                            </TableRow>
                        )}
                    </TableBody>
                </Table>
